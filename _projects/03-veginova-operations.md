...
tagline: "What to produce, how much, and when. A seasonal forecast built from history instead of gut feel, feeding the production plan."
description: "The production plan lived in a spreadsheet. It went stale the moment a sale landed. That is dangerous when seed takes a year to grow. I rebuilt the plan as a live system. It matches the planner's own numbers exactly, warns about shortages while there is still time to act, and lets them test a big order before committing seed."
tools: [Power BI, PostgreSQL, Python, Supabase]
outcome_headline: "Matched the planner's existing production numbers exactly, with zero mismatches against their spreadsheet, then made the plan live and testable"
outcome_detail: "The system agrees with the hand-built spreadsheet on every variety it was checked against. It works out what to produce from live sales, stock, and incoming orders. It flags shortages early enough to act on a one-year lead time, and lets the planner test a big sale or a capacity drop before any seed is committed."
order: 2
...
## Action: put the planner's own calculation into a system that updates itself
...
The path from source to screen: sales, stock, and incoming orders load automatically from the source sheets into Postgres with Python. The planning logic runs as one SQL view against a proper data model. Power BI is the display layer on top. Building it this way is what lets the plan update itself and stay accurate as sales and stock change.
